...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # Note: I tried to loop through each ghost in livingGhosts. This is wrong. When I tried to loop through each
        # distribution in the distribution list, this turns out to be correct. Why is there a difference here?
        ghost_positions = []
        for ghost_distribution in livingGhostPositionDistributions:
            ghost_positions.append(ghost_distribution.argMax())

        # Find the closest ghost to pacman
        closest_ghost = None
        closest_ghost_dist = float("inf")
        for ghost_position in ghost_positions:
            ghost_dist = self.distancer.getDistance(pacmanPosition, ghost_position)
            if ghost_dist < closest_ghost_dist:
                closest_ghost_dist = ghost_dist
        ...
        for action in legal:
            ...
            new_distance = self.distancer.getDistance(new_position, closest_ghost)
            if new_distance < closest_ghost_dist:
                closest_ghost_dist = new_distance
                best_action = action
        return best_action
