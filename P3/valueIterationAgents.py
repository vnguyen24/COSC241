...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for i in range(self.iterations):
            ...
            for state in mdp.getStates():
                ...
                for action in actions:
                    # Note: There is no way to get the reward of current state, so I included reward of next state in
                    # the QValue computation
            ...
            # Q: I need U <- U' down here for code to work. Why?
            self.values = U_prime.copy()
    ...
    def computeQValueFromValues(self, state, action):
        ...
        the next k+1 rewards (i.e. you return πk+1πk+1). Similarly, the Q-values will also reflect one more reward 
        than the values (i.e. you return Qk+1). You should return the synthesized policy πk+1πk+1. Therefore, 
        instead of calculating R(s) + discount * ..., we calculate P(s'|s,a) * ((R(s,a,s') + discount * U[s']) 
        instead """
        running_sum = 0
        for next_state, prob in next_states_and_probs:
            running_sum += prob * (mdp.getReward(state, action, next_state) + discount * self.getValue(next_state))
